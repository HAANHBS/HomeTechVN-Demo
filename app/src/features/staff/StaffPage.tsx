...
function StaffForm({
  ...
  const [roleId, setRoleId] = useState(initial?.role_id ?? roles.find((role) => role.code !== 'admin')?.id ?? roles[0]?.id ?? '')
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError(null)
    try {
      if (!roleId) throw new Error('Hãy chọn vai trò cho nhân viên.')
      if (initial) {
  ...
  return <form className="space-y-4" onSubmit={submit}>
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <label className="text-sm font-medium">Vai trò *<select required disabled={protectAccess} className={inputClass} value={roleId} onChange={(event) => setRoleId(event.target.value)}>{roles.filter((role) => role.is_active || role.id === roleId).map((role) => <option key={role.id} value={role.id}>{viRole(role.code, role.name)}</option>)}</select></label>
...
export function StaffPage({ context }: { context: AppUserContext }) {
  ...
  const filtered = useMemo(() => {
    ...
    return staff.filter((row) => {
      if (roleFilter !== 'ALL' && row.role_id !== roleFilter) return false
      ...
      return [row.full_name ?? '', row.email ?? '', row.phone ?? '', role?.name ?? '', role?.code ?? ''].join(' ').toLocaleLowerCase('vi-VN').includes(query)
    })
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-200">
    <header className="border-b border-slate-800 bg-slate-900/90 px-4 py-4 sm:px-6"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4"><div><div className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-400">HomeTechVN</div><h1 className="mt-1 text-xl font-bold text-white">Nhân viên & phân quyền</h1></div><button onClick={() => void supabase.auth.signOut()} className="rounded-xl border border-slate-700 px-3 py-2 text-sm">Đăng xuất</button></div></header>
