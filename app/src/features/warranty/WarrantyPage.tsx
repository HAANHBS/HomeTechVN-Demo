  ...
  return <section className="rounded-2xl border border-cyan-900/70 bg-cyan-950/10 p-4 sm:p-5">
    ...
    {result?.found?<div className={`mt-4 rounded-2xl border p-4 ${positive?'border-emerald-800 bg-emerald-950/25':'border-amber-800 bg-amber-950/20'}`}>
      ...
      {result.has_warranty?<div className="mt-3 grid gap-3 lg:grid-cols-2">{result.matches.map((match)=><article key={match.warranty_id} className="rounded-xl border border-slate-700 bg-slate-950/60 p-3 text-sm">
        ...
        <div className="mt-3 space-y-1 text-slate-400"><div>Số sê-ri: <span className="font-mono text-slate-200">{match.serial_number??'—'}</span></div><div>Khách: <span className="text-slate-200">{match.customer_name}</span></div><div>Đơn/nguồn: <span className="font-mono text-slate-200">{match.source_code??'—'}</span></div><div>Hiệu lực: {dateOnly(match.start_date)} → {dateOnly(match.end_date)}{match.effective_status==='ACTIVE'?` · còn ${match.days_remaining} ngày`:''}</div>{match.has_open_claim?<div className="text-amber-300">Đang có yêu cầu {match.latest_claim_code??''} · {viStatus(match.latest_claim_status)}</div>:null}</div>
...
export function WarrantyPage({
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-200">
    <header className="border-b border-slate-800 bg-slate-900/90 px-4 py-4 sm:px-6"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4"><div><div className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-400">HomeTechVN</div><h1 className="mt-1 text-xl font-bold text-white">Bảo hành & yêu cầu xử lý</h1></div><div className="flex flex-wrap items-center gap-2 text-sm">{onOpenCrm?<button onClick={onOpenCrm} className="rounded-xl border border-cyan-900 px-3 py-2 text-cyan-300">Khách hàng</button>:null}{onOpenInventory?<button onClick={onOpenInventory} className="rounded-xl border border-cyan-900 px-3 py-2 text-cyan-300">Kho</button>:null}{onOpenSales?<button onClick={onOpenSales} className="rounded-xl border border-cyan-900 px-3 py-2 text-cyan-300">Bán hàng</button>:null}{onOpenRepair?<button onClick={onOpenRepair} className="rounded-xl border border-cyan-900 px-3 py-2 text-cyan-300">Sửa chữa</button>:null}{onOpenChecklist?<button onClick={onOpenChecklist} className="rounded-xl border border-cyan-900 px-3 py-2 text-cyan-300">Danh sách kiểm tra</button>:null}<div className="px-2 text-right"><div className="font-medium text-white">{context.fullName||context.email||'Người dùng'}</div><div className="text-xs text-slate-500">{context.roleName} · {context.roleCode}</div></div><button onClick={() => void supabase.auth.signOut()} className="rounded-xl border border-slate-700 px-3 py-2">Đăng xuất</button></div></div></header>
