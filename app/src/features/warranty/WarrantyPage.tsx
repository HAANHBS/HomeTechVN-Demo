  ...
  return <section className="rounded-2xl border border-cyan-900/70 bg-cyan-950/10 p-4 sm:p-5">
    ...
    {result?.found?<div className={`mt-4 rounded-2xl border p-4 ${positive?'border-emerald-800 bg-emerald-950/25':'border-amber-800 bg-amber-950/20'}`}>
      ...
      {result.has_warranty?<div className="mt-3 grid gap-3 lg:grid-cols-2">{result.matches.map((match)=><article key={match.warranty_id} className="rounded-xl border border-slate-700 bg-slate-950/60 p-3 text-sm">
        ...
        <div className="mt-3 space-y-1 text-slate-400"><div>Serial: <span className="font-mono text-slate-200">{match.serial_number??'—'}</span></div><div>Khách: <span className="text-slate-200">{match.customer_name}</span></div><div>Đơn/nguồn: <span className="font-mono text-slate-200">{match.source_code??'—'}</span></div><div>Hiệu lực: {dateOnly(match.start_date)} → {dateOnly(match.end_date)}{match.effective_status==='ACTIVE'?` · còn ${match.days_remaining} ngày`:''}</div>{match.has_open_claim?<div className="text-amber-300">Đang có claim {match.latest_claim_code??''} · {match.latest_claim_status??''}</div>:null}</div>
