...
function moduleLabel(value: string | null | undefined) {
  const labels: Record<string, string> = { SALES: 'Bán hàng', REPAIR: 'Sửa chữa', WARRANTY: 'Bảo hành', SERVICE: 'Dịch vụ', GENERIC: 'Dùng chung' }
  return value ? labels[value] ?? value : '—'
}

function entityLabel(value: string | null | undefined) {
  const labels: Record<string, string> = { SALES_ORDER: 'Đơn bán', REPAIR_ORDER: 'Phiếu sửa chữa', WARRANTY: 'Bảo hành', SERVICE: 'Dịch vụ', GENERIC: 'Đối tượng chung' }
  return value ? labels[value] ?? value : '—'
}

function requirementLabel(value: string | null | undefined) {
  const labels: Record<string, string> = { ALWAYS: 'Luôn bắt buộc', OPTIONAL: 'Tùy chọn', SALES_HAS_SERIAL: 'Bắt buộc khi đơn có số sê-ri' }
  return value ? labels[value] ?? value : '—'
}
...
function StartRunForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!id) throw new Error('Hệ thống không trả mã lần kiểm tra.')
      onStarted(id)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Không tạo được lần kiểm tra.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return <form className="space-y-4" onSubmit={submit}>
    <label className="block text-sm font-medium">Mẫu kiểm tra
    ...
    {template?.entity_type === 'SALES_ORDER' ? <label className="block text-sm font-medium">Đơn bán
      <select className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2" value={entityId} onChange={(e) => setEntityId(e.target.value)} required>
        ...
        {sales.filter((x) => x.id).map((x) => <option key={x.id!} value={x.id!}>{x.order_code} · {x.customer_name} · {viStatus(x.status)}</option>)}
      </select>
    </label> : null}
    ...
    {template?.entity_type === 'REPAIR_ORDER' ? <label className="block text-sm font-medium">Phiếu sửa chữa
      <select className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2" value={entityId} onChange={(e) => setEntityId(e.target.value)} required>
        ...
        {repairs.filter((x) => x.id).map((x) => <option key={x.id!} value={x.id!}>{x.repair_code} · {x.customer_name} · {viStatus(x.status)}</option>)}
      </select>
    </label> : null}

    {template?.entity_type === 'GENERIC' ? <label className="block text-sm font-medium">Mã đối tượng UUID
    ...
    <div className="flex justify-end gap-2">
      ...
      <button disabled={busy || !templateId || !entityId} className="rounded-xl bg-cyan-500 px-4 py-2 font-semibold text-slate-950 disabled:opacity-50">{busy ? 'Đang tạo…' : 'Bắt đầu kiểm tra'}</button>
    </div>
...
function CreateTemplateForm({ onCancel, onCreated }: { onCancel: () => void; onCreated: (id: string) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (rpcError) throw rpcError
      const id = jsonId(data)
      if (!id) throw new Error('Hệ thống không trả mã mẫu kiểm tra.')
      onCreated(id)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Không tạo được mẫu kiểm tra.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return <form className="space-y-4" onSubmit={submit}>
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-medium">Mã mẫu
      ...
      <label className="text-sm font-medium">Phân hệ
        <select className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2" value={module} onChange={(e) => setModule(e.target.value)}>
          {['SALES','REPAIR','WARRANTY','SERVICE','GENERIC'].map((x) => <option key={x} value={x}>{moduleLabel(x)}</option>)}
        </select>
      </label>
      <label className="text-sm font-medium">Loại đối tượng
        <select className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2" value={entityType} onChange={(e) => setEntityType(e.target.value)}>
          {['SALES_ORDER','REPAIR_ORDER','WARRANTY','SERVICE','GENERIC'].map((x) => <option key={x} value={x}>{entityLabel(x)}</option>)}
        </select>
      </label>
    </div>
    ...
    <div className="flex justify-end gap-2">
      <button type="button" onClick={onCancel} className="rounded-xl border border-slate-700 px-4 py-2">Đóng</button>
      <button disabled={busy} className="rounded-xl bg-cyan-500 px-4 py-2 font-semibold text-slate-950 disabled:opacity-50">{busy ? 'Đang tạo…' : 'Tạo phiên bản mới'}</button>
    </div>
    <ErrorPanel message={error} />
  </form>
}
...
function AddTemplateItemForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Không thêm được mục kiểm tra.')
    } finally {
      setBusy(false)
    }
  }
  ...
  return <form className="space-y-4" onSubmit={submit}>
    <div className="grid gap-4 sm:grid-cols-2">
      <label className="text-sm font-medium">Mã mục
    ...
    <label className="block text-sm font-medium">Mức bắt buộc
      <select className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2" value={rule} onChange={(e) => setRule(e.target.value)}>
        <option value="ALWAYS">Luôn bắt buộc</option>
        <option value="OPTIONAL">Tùy chọn</option>
        {template.entity_type === 'SALES_ORDER' ? <option value="SALES_HAS_SERIAL">Bắt buộc khi đơn có số sê-ri</option> : null}
      </select>
    </label>
    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={systemManaged} onChange={(e) => setSystemManaged(e.target.checked)} /> Hệ thống tự quản lý trạng thái mục</label>
    <label className="block text-sm font-medium">Mô tả
      <textarea className="mt-2 min-h-20 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2" value={description} onChange={(e) => setDescription(e.target.value)} />
    </label>
    <div className="flex justify-end gap-2">
      <button type="button" onClick={onCancel} className="rounded-xl border border-slate-700 px-4 py-2">Đóng</button>
      <button disabled={busy} className="rounded-xl bg-cyan-500 px-4 py-2 font-semibold text-slate-950 disabled:opacity-50">{busy ? 'Đang thêm…' : 'Thêm mục'}</button>
    </div>
    <ErrorPanel message={error} />
  </form>
}
...
function RunDetail({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Không tải được lần kiểm tra.')
    }
  }, [runId])
  ...
  async function reopen() {
    const note = window.prompt('Ghi chú mở lại danh sách kiểm tra:') ?? ''
  ...
  async function cancel() {
    const note = window.prompt('Lý do hủy lần kiểm tra:')
    if (!note?.trim()) return
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <button onClick={onBack} className="text-sm text-cyan-300 hover:underline">← Danh sách kiểm tra</button>
        <h2 className="mt-2 text-2xl font-bold text-white">{run.template_code_snapshot} v{run.template_version}</h2>
        <p className="mt-1 font-mono text-xs text-slate-500">{entityLabel(run.entity_type)} · {run.entity_id}</p>
      </div>
      <span title={run.status} className={`rounded-xl px-3 py-2 text-sm font-semibold ${statusClass(run.status)}`}>{viStatus(run.status)}</span>
    </div>
    ...
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-xl border border-slate-800 bg-slate-900 p-4"><div className="text-xs text-slate-500">Tổng mục</div><div className="mt-1 text-xl font-bold text-white">{items.filter((x) => x.checked).length}/{items.length}</div></div>
    ...
    <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-800 bg-slate-900 p-4">
      ...
      {run.status !== 'CANCELLED' && canManage ? <button onClick={() => void cancel()} className="rounded-xl border border-red-900 px-3 py-2 text-sm text-red-300">Hủy lần kiểm tra</button> : null}
    </div>
    <ErrorPanel message={error} />

    {run.status === 'OPEN' && requiredDone < required.length ? <div className="rounded-xl border border-amber-900/70 bg-amber-950/20 p-3 text-sm text-amber-100"><strong>Chưa thể hoàn tất:</strong> còn {required.length-requiredDone} mục bắt buộc. Hãy hoàn thành các mục chưa đánh dấu bên dưới; mục do hệ thống quản lý phải hoàn tất ở đúng bộ phận nghiệp vụ.</div> : null}
    ...
    <section className="space-y-2">
      {items.map((item) => <label key={item.id} className={`flex items-start gap-3 rounded-xl border p-4 ${item.checked ? 'border-emerald-900 bg-emerald-950/20' : 'border-slate-800 bg-slate-900'}`}>
        ...
        <span className="min-w-0 flex-1">
          ...
          {item.system_managed ? <span className="ml-2 rounded bg-violet-950 px-1.5 py-0.5 text-[10px] text-violet-300">HỆ THỐNG</span> : null}
...
function TemplatePanel({
  ...
  return <div className="space-y-5">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><button onClick={onBack} className="text-sm text-cyan-300">← Mẫu kiểm tra</button><h2 className="mt-2 text-2xl font-bold text-white">{template.template_code} v{template.version}</h2><p className="text-sm text-slate-500">{template.name} · {moduleLabel(template.module)} · {entityLabel(template.entity_type)}</p></div>
      <div className="flex gap-2">{template.is_system ? <span className="rounded-xl bg-violet-950 px-3 py-2 text-xs text-violet-300">HỆ THỐNG</span> : null}{template.is_active ? <span className="rounded-xl bg-emerald-950 px-3 py-2 text-xs text-emerald-300">ĐANG DÙNG</span> : <button onClick={() => void activate()} className="rounded-xl bg-emerald-500 px-3 py-2 text-sm font-semibold text-slate-950">Kích hoạt</button>}</div>
    </div>

    {!template.is_active ? <button onClick={() => setShowAdd(true)} className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950">+ Thêm mục</button> : null}
    ...
    <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
      <div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="bg-slate-950/60 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">#</th><th className="px-4 py-3">Key / Nội dung</th><th className="px-4 py-3">Rule</th><th className="px-4 py-3">System</th></tr></thead><tbody>
        {items.map((i) => <tr key={i.id} className="border-t border-slate-800"><td className="px-4 py-3">{i.sort_order}</td><td className="px-4 py-3"><div className="font-medium text-white">{i.label}</div><div className="font-mono text-xs text-cyan-400">{i.item_key}</div></td><td className="px-4 py-3">{requirementLabel(i.requirement_rule)}</td><td className="px-4 py-3">{i.system_managed ? '✓' : '—'}</td></tr>)}
      </tbody></table></div>
    </div>
    <ErrorPanel message={error} />
    {showAdd ? <Modal title="Thêm mục kiểm tra" onClose={() => setShowAdd(false)}><AddTemplateItemForm template={template} nextSort={(items.at(-1)?.sort_order ?? 0) + 1} onCancel={() => setShowAdd(false)} onDone={() => { setShowAdd(false); void load() }} /></Modal> : null}
  </div>
}
...
export function ChecklistPage({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Không tải được hệ thống danh sách kiểm tra.')
    }
  }, [])
  ...
  if (!canRun && !canManage) {
    return <main className="grid min-h-screen place-items-center bg-slate-950 text-slate-200"><div className="rounded-2xl border border-amber-900 p-6">Vai trò hiện tại không có quyền sử dụng danh sách kiểm tra.</div></main>
  }
  ...
  return <main className="min-h-screen bg-slate-950 text-slate-200">
    <header className="border-b border-slate-800 bg-slate-900/90 px-4 py-4 sm:px-6">
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4">
        <div><div className="text-sm font-semibold uppercase tracking-[0.24em] text-cyan-400">HomeTechVN</div><h1 className="mt-1 text-xl font-bold text-white">Danh sách kiểm tra</h1></div>
       ...
       <div className="space-y-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex gap-2">
            <button onClick={() => setTab('runs')} className={`rounded-xl px-4 py-2 text-sm ${tab === 'runs' ? 'bg-cyan-500 font-semibold text-slate-950' : 'border border-slate-700'}`}>Lần kiểm tra</button>
            {canManage ? <button onClick={() => setTab('templates')} className={`rounded-xl px-4 py-2 text-sm ${tab === 'templates' ? 'bg-cyan-500 font-semibold text-slate-950' : 'border border-slate-700'}`}>Mẫu kiểm tra</button> : null}
          </div>
          <div className="flex gap-2">
            <button onClick={() => void load()} className="rounded-xl border border-slate-700 px-4 py-2 text-sm">Làm mới</button>
            {tab === 'runs' && canRun ? <button onClick={() => setShowStart(true)} className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950">+ Tạo mới</button> : null}
            {tab === 'templates' && canManage ? <button onClick={() => setShowCreate(true)} className="rounded-xl bg-cyan-500 px-4 py-2 text-sm font-semibold text-slate-950">+ Tạo mới</button> : null}
          </div>
        </div>
        ...
        {tab === 'runs' ? <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
          <div className="overflow-x-auto"><table className="w-full min-w-[1000px] text-left text-sm"><thead className="bg-slate-950/60 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Mẫu</th><th className="px-4 py-3">Đối tượng</th><th className="px-4 py-3">Tiến độ</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3">Bắt đầu</th><th className="px-4 py-3 text-right">Mở</th></tr></thead><tbody>
            {runs.map((r) => r.id ? <tr key={r.id} className="border-t border-slate-800 hover:bg-slate-800/30"><td className="px-4 py-3"><div className="font-medium text-white">{r.template_name}</div><div className="font-mono text-xs text-cyan-400">{r.template_code_snapshot} v{r.template_version}</div></td><td className="px-4 py-3"><div>{entityLabel(r.entity_type)}</div><div className="max-w-xs truncate font-mono text-[11px] text-slate-500">{r.entity_id}</div></td><td className="px-4 py-3">{r.required_checked_count ?? 0}/{r.required_count ?? 0} bắt buộc · {r.checked_count ?? 0}/{r.item_count ?? 0} tổng</td><td className="px-4 py-3"><span title={r.status ?? undefined} className={`rounded-lg px-2 py-1 text-xs ${statusClass(r.status)}`}>{viStatus(r.status)}</span></td><td className="px-4 py-3 text-slate-400">{dateTime(r.started_at)}</td><td className="px-4 py-3 text-right"><button onClick={() => setRunId(r.id!)} className="rounded-lg border border-slate-700 px-3 py-1 text-xs">Chi tiết</button></td></tr> : null)}
          </tbody></table></div>
          {runs.length === 0 ? <p className="p-8 text-center text-slate-500">Chưa có lần kiểm tra.</p> : null}
        </div> : null}

        {tab === 'templates' && canManage ? <div className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900">
          <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="bg-slate-950/60 text-xs uppercase text-slate-500"><tr><th className="px-4 py-3">Mã</th><th className="px-4 py-3">Tên</th><th className="px-4 py-3">Phân hệ / đối tượng</th><th className="px-4 py-3">Trạng thái</th><th className="px-4 py-3 text-right">Mở</th></tr></thead><tbody>
            {templates.map((t) => <tr key={t.id} className="border-t border-slate-800"><td className="px-4 py-3 font-mono text-cyan-300">{t.template_code} v{t.version}</td><td className="px-4 py-3">{t.name}{t.is_system ? <span className="ml-2 rounded bg-violet-950 px-1.5 py-0.5 text-[10px] text-violet-300">HỆ THỐNG</span> : null}</td><td className="px-4 py-3">{moduleLabel(t.module)} · {entityLabel(t.entity_type)}</td><td className="px-4 py-3">{t.is_active ? <span className="text-emerald-300">Đang dùng</span> : <span className="text-slate-500">Bản nháp/cũ</span>}</td><td className="px-4 py-3 text-right"><button onClick={() => setTemplateId(t.id)} className="rounded-lg border border-slate-700 px-3 py-1 text-xs">Chi tiết</button></td></tr>)}
          </tbody></table></div>
        </div> : null}
        <ErrorPanel message={error} />
       </div>}
    ...
    {showStart ? <Modal title="Bắt đầu lần kiểm tra" onClose={() => setShowStart(false)}><StartRunForm templates={templates} sales={sales} repairs={repairs} onCancel={() => setShowStart(false)} onStarted={(id) => { setShowStart(false); setRunId(id); void load() }} /></Modal> : null}
    {showCreate ? <Modal title="Tạo phiên bản mẫu" onClose={() => setShowCreate(false)}><CreateTemplateForm onCancel={() => setShowCreate(false)} onCreated={(id) => { setShowCreate(false); void load(); setTemplateId(id) }} /></Modal> : null}
  </main>
}
