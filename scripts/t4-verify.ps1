...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T4:`n- " + ($unexpected -join "`n- ")) }
    Write-Host '[PASS] Migration T1+T2+T3+T4: 11/11'
    return $actual
}
...
function Invoke-T4SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)
    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t4_verify.sql'
    if (-not (Test-Path $verifyFile)) { throw "Khong tim thay $verifyFile" }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t4_verify.sql"
    ...
    if ($exitCode -ne 0) { throw "T4 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T4 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T4 PASS.' }
    return $text
}

try {
    Write-Host '=== HomeTechVN T4 - Sales Verification ==='
    ...
    $sqlOutput = Invoke-T4SqlVerify -Container $container
    ...
    $snapshotPath = Join-Path $snapshotDir ('T4_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')
    $snapshot = @(
        'HomeTechVN T4 local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T4 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T4 FINAL CORE CHECKS: PASS'
    Write-Host 'T4 APP BUILD: PASS'
    ...
    Write-Host ''
    Write-Host '[T4 FAIL]' -ForegroundColor Red
