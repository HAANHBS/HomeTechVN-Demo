...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T2:`n- " + ($unexpected -join "`n- ")) }
    Write-Host '[PASS] Migration T1+T2: 6/6'
    return $actual
}
...
function Invoke-T2SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)
    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t2_verify.sql'
    if (-not (Test-Path $verifyFile)) { throw "Khong tim thay $verifyFile" }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t2_verify.sql"
    ...
    if ($exitCode -ne 0) { throw "T2 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T2 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T2 PASS.' }
    return $text
}

try {
    Write-Host '=== HomeTechVN T2 - DB + App Verification ==='
    ...
    $sqlOutput = Invoke-T2SqlVerify -Container $container
    ...
    $snapshotPath = Join-Path $snapshotDir ('T2_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')
    $snapshot = @(
        'HomeTechVN T2 local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T2 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T2 FINAL CORE CHECKS: PASS'
    Write-Host 'T2 APP BUILD: PASS'
    ...
    Write-Host ''
    Write-Host '[T2 FAIL]' -ForegroundColor Red
