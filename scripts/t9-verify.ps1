...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T9:`n- " + ($unexpected -join "`n- ")) }
    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6+T7+T8+T9: 27/27'
    return $actual
}
...
function Invoke-T9SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)
    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t9_verify.sql'
    if (-not (Test-Path $verifyFile)) { throw "Khong tim thay $verifyFile" }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t9_verify.sql"
    ...
    foreach ($line in $output) { Write-Host $line }
    $text = $output -join [Environment]::NewLine
    if ($exitCode -ne 0) { throw "T9 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T9 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T9 PASS.' }
    return $text
}

try {
    Write-Host '=== HomeTechVN T9 v1.0 - Reminder Engine Verification ==='
    ...
    $sqlOutput = Invoke-T9SqlVerify -Container $container
    ...
    $snapshotPath = Join-Path $snapshotDir ('T9_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')
    $snapshot = @(
        'HomeTechVN T9 local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T9 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T9 FINAL CORE CHECKS: PASS'
    Write-Host 'T9 APP BUILD: PASS'
    ...
    Write-Host ''
    Write-Host '[T9 FAIL]' -ForegroundColor Red
