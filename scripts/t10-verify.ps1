...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) {
        throw ("Co migration ngoai checkpoint T10:`n- " + ($unexpected -join "`n- "))
    }

    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6+T7+T8+T9+T10: 30/30'
    return $actual
}
...
function Invoke-T10SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)

    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t10_verify.sql'
    if (-not (Test-Path $verifyFile)) {
        throw "Khong tim thay $verifyFile"
    }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t10_verify.sql"
    ...
    if ($exitCode -ne 0) {
        throw "T10 SQL verify fail, psql exit=$exitCode`n$text"
    }
    if ($text -notmatch 'T10 FINAL CORE CHECKS: PASS') {
        throw 'Khong tim thay marker T10 FINAL CORE CHECKS: PASS.'
    }
    return $text
}

try {
    Write-Host '=== HomeTechVN T10 v1.0 - Notification + Zalo Verification ==='
    ...
    Write-Host '[INFO] Dong bo Worker dependencies + package-lock.json'
    Invoke-CheckedCommand -FilePath $npm -Arguments @(
        '--prefix','worker','install','--no-audit','--no-fund'
    ) | Out-Null

    Invoke-CheckedCommand -FilePath $node -Arguments @(
        '--check',
        (Join-Path $ProjectRoot 'worker\src\index.js')
    ) | Out-Null

    $workerDryRun = Join-Path $ProjectRoot 'worker\.t10-dryrun'
    if (Test-Path $workerDryRun) {
        Remove-Item -Path $workerDryRun -Recurse -Force
    }

    Push-Location (Join-Path $ProjectRoot 'worker')
    try {
        Invoke-CheckedCommand -FilePath $npx -Arguments @(
            'wrangler','deploy','--dry-run','--outdir','.t10-dryrun'
        ) | Out-Null
    }
    finally {
        Pop-Location
    ...
    if (-not (Test-Path $workerDryRun)) {
        throw 'Wrangler dry-run khong tao output bundle.'
    }
    Remove-Item -Path $workerDryRun -Recurse -Force

    Write-Host '[PASS] Cloudflare Worker syntax + Wrangler dry-run bundle'

    $snapshotDir = Join-Path $ProjectRoot 'docs\snapshots'
    New-Item -ItemType Directory -Path $snapshotDir -Force | Out-Null
    $snapshotPath = Join-Path $snapshotDir (
        'T10_LOCAL_VERIFY_' + (Get-Stamp) + '.txt'
    )

    $workerVersion = (
        Get-Content (Join-Path $ProjectRoot 'worker\package.json') -Raw |
        ConvertFrom-Json
    ).devDependencies.wrangler

    $snapshot = @(
        'HomeTechVN T10 local verification',
        ('Checked: ' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz')),
        ('Project: ' + $projectId),
        ('Wrangler pinned: ' + $workerVersion),
        '',
        'Migrations:',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T10 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T10 FINAL CORE CHECKS: PASS'
    Write-Host 'T10 APP BUILD: PASS'
    Write-Host 'T10 WORKER CHECK: PASS'
    ...
    Write-Host ''
    Write-Host '[T10 FAIL]' -ForegroundColor Red
