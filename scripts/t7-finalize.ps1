...
function Read-ProjectId {
    ...
    if (-not (Test-Path $configPath)) { throw 'Khong tim thay supabase/config.toml.' }
...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T7:`n- " + ($unexpected -join "`n- ")) }

    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6+T7: 21/21'
    return $actual
}

function Invoke-T7SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)

    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t7_verify.sql'
    if (-not (Test-Path $verifyFile)) { throw "Khong tim thay $verifyFile" }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t7_verify.sql"
    ...
    if ($exitCode -ne 0) { throw "T7 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T7 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T7 PASS.' }

    return $text
}

try {
    Write-Host '=== HomeTechVN T7 v1.1 - Frontend Finalize ==='
    ...
    # Non-destructive: this verifier is transaction/ROLLBACK based.
    $sqlOutput = Invoke-T7SqlVerify -Container $container

    Write-Host '[INFO] Dong bo app dependencies + package-lock.json'
    Invoke-CheckedCommand -FilePath $npm -Arguments @('--prefix','app','install','--no-audit','--no-fund')
    Invoke-CheckedCommand -FilePath $npm -Arguments @('--prefix','app','run','build')
    Write-Host '[PASS] React/TypeScript/Vite production build'

    $snapshotDir = Join-Path $ProjectRoot 'docs\snapshots'
    New-Item -ItemType Directory -Path $snapshotDir -Force | Out-Null
    $snapshotPath = Join-Path $snapshotDir ('T7_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')

    $snapshot = @(
        'HomeTechVN T7 final local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T7 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T7 FINAL CORE CHECKS: PASS'
    Write-Host 'T7 APP BUILD: PASS'
    ...
    Write-Host ''
    Write-Host '[T7 FAIL]' -ForegroundColor Red
