...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T11:`n- " + ($unexpected -join "`n- ")) }
    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6+T7+T8+T9+T10+T11: 31/31'
    return $actual
}
...
function Invoke-T11SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)
    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t11_verify.sql'
    ...
    if ($exitCode -ne 0) { throw "T11 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T11 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T11 FINAL CORE CHECKS: PASS.' }
    return $text
}

try {
    Write-Host '=== HomeTechVN T11 v1.0 - Dashboard + Responsive UI Verification ==='
    ...
    $snapshotPath = Join-Path $snapshotDir ('T11_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')
    $snapshot = @(
        'HomeTechVN T11 local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T11 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T11 FINAL CORE CHECKS: PASS'
    Write-Host 'T11 RESPONSIVE UI CHECK: PASS'
    Write-Host 'T11 APP BUILD: PASS'
    Write-Host "Snapshot: $snapshotPath"
    Write-Host '=========================================='
    exit 0
} catch {
    Write-Host ''
    Write-Host '[T11 FAIL]' -ForegroundColor Red
