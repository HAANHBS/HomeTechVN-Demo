...
function localConfig(){
  ...
  if(!['127.0.0.1','localhost','::1'].includes(host))throw new Error(`T19 refuses non-local Supabase URL: ${value.apiUrl}`)
  return value
}
...
async function verifyQr(local,container){
  sqlFile(container,'supabase/tests/t19_verify.sql','T19 QR DATABASE SECURITY CHECK: PASS')
  ...
  if(!customer?.customer_code||!order?.order_code)throw new Error('T19 demo QR targets are missing')
...
async function main(){
  try{
    ...
    run(process.execPath,[path.join('scripts','t19-source-check.mjs')])
    run(npm,['ci','--no-audit','--no-fund'])
    run(npm,['--prefix','app','ci','--no-audit','--no-fund'])
    run(npm,['--prefix','worker','ci','--no-audit','--no-fund'])
    console.log('T19 LOCAL REPRODUCIBILITY: PASS')
    ...
    console.log('T19 APP BUILD: PASS')
    run(npm,['--prefix','worker','run','check'])
    console.log('T19 WORKER CHECK: PASS')
  ...
  try{
    ...
    console.log('T19 CLEAN BASELINE AFTER VERIFY: PASS')
