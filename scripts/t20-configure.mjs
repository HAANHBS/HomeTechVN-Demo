...
function renderEnv(url, key) {
  return [
    '# HomeTechVN T20 hosted-demo working-copy configuration.',
...
function selfTest() {
  ...
  if (!rendered.includes('VITE_HOMETECHVN_HOSTED_DEMO=true\r\n')) {
    throw new Error('configure renderer did not emit the hosted-demo marker')
  }
  for (const bad of [
  ...
  console.log('T20 CONFIGURE SELF TEST: PASS')
}
...
async function main() {
  ...
  if (fs.existsSync(target) && !process.argv.includes('--force')) {
    ...
    console.log('Run npm run t20:env-check to validate it, or rerun with -- --force to replace it.')
    return
  }

  console.log('HomeTechVN T20 hosted frontend configuration')
  ...
  console.log('T20 HOSTED FRONTEND CONFIG: SAVED')
  console.log('T20 PRODUCTION CONFIG SAFETY: PASS')
  console.log('Saved app/.env.local locally; release packaging will exclude it.')
}

main().catch((error) => {
  console.error(`[T20 CONFIGURE FAIL] ${error.message}`)
  process.exit(1)
})
