...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) {
        throw ("T14 khong duoc them/sua migration database. Co file ngoai baseline:`n- " + ($unexpected -join "`n- "))
    }
    if (@($actual | Where-Object { $_ -match '_t14_' }).Count -gt 0) {
        throw 'T14 PWA khong can migration database.'
    }
    Write-Host '[PASS] Database migration baseline T1-T13 remains 33/33; T14 adds 0 migration.'
    return $actual
}
...
try {
    Write-Host '=== HomeTechVN T14 v1.0 - PWA Verification ==='
    ...
    Write-Host '[PASS] T11-T14 responsive/privacy/PWA source regression checks'

    Invoke-CheckedCommand -FilePath $npm -Arguments @('--prefix','app','install','--no-audit','--no-fund') | Out-Null
    Invoke-CheckedCommand -FilePath $npm -Arguments @('--prefix','app','ls','vite-plugin-pwa','--depth=0') | Out-Null
    Invoke-CheckedCommand -FilePath $npm -Arguments @('--prefix','app','run','build') | Out-Null
    Write-Host '[PASS] React/TypeScript/Vite PWA production build'

    Invoke-CheckedCommand -FilePath $node -Arguments @((Join-Path $ProjectRoot 'scripts\t14-build-check.mjs')) | Out-Null
    Write-Host '[PASS] Built manifest/service-worker/icons/offline policy checks'

    $snapshotDir = Join-Path $ProjectRoot 'docs\snapshots'
    New-Item -ItemType Directory -Path $snapshotDir -Force | Out-Null
    $snapshotPath = Join-Path $snapshotDir ('T14_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')

    $snapshot = @(
        'HomeTechVN T14 local verification',
        ...
        $sqlOutput,
        '',
        'T11 GLOBAL RESPONSIVE REGRESSION: PASS',
        'T12 PUBLIC WARRANTY REGRESSION: PASS',
        'T13 REPORT REGRESSION: PASS',
        'T14 PWA SOURCE CHECK: PASS',
        'T14 PWA BUILD CHECK: PASS',
        'APP BUILD: PASS',
        '',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T14 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T14 PWA CORE CHECKS: PASS'
    Write-Host 'T14 RESPONSIVE UI CHECK: PASS'
    Write-Host 'T14 APP BUILD: PASS'
    Write-Host "Snapshot: $snapshotPath"
    Write-Host '=========================================='
    exit 0
} catch {
    Write-Host ''
    Write-Host '[T14 FAIL]' -ForegroundColor Red
