...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T8:`n- " + ($unexpected -join "`n- ")) }
    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6+T7+T8: 23/23'
    return $actual
}
...
function Invoke-T8SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)
    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t8_verify.sql'
    if (-not (Test-Path $verifyFile)) { throw "Khong tim thay $verifyFile" }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t8_verify.sql"
    ...
    foreach ($line in $output) { Write-Host $line }
    $text = $output -join [Environment]::NewLine
    if ($exitCode -ne 0) { throw "T8 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T8 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T8 PASS.' }
    return $text
}

try {
    Write-Host '=== HomeTechVN T8 v1.0 - Service + License Verification ==='
    ...
    $sqlOutput = Invoke-T8SqlVerify -Container $container
    ...
    $snapshotPath = Join-Path $snapshotDir ('T8_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')
    $snapshot = @(
        'HomeTechVN T8 local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T8 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T8 FINAL CORE CHECKS: PASS'
    Write-Host 'T8 APP BUILD: PASS'
    ...
    Write-Host ''
    Write-Host '[T8 FAIL]' -ForegroundColor Red
