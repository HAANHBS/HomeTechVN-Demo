...
if (rootPackage.version !== '0.21.1-t21.1' || rootLock.version !== rootPackage.version || rootLock.packages?.['']?.version !== rootPackage.version) {
  fail('root package/lock version mismatch')
}
if (appPackage.version !== '0.21.1' || appLock.version !== appPackage.version || appLock.packages?.['']?.version !== appPackage.version) {
  fail('app package/lock version mismatch')
}
for (const script of ['t21:ui-check', 't21:verify']) {
