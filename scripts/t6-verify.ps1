...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) { throw ("Co migration ngoai checkpoint T6:`n- " + ($unexpected -join "`n- ")) }
    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6: 17/17'
    return $actual
}
...
function Invoke-T6SqlVerify {
    param([Parameter(Mandatory=$true)][string]$Container)
    $verifyFile = Join-Path $ProjectRoot 'supabase\tests\t6_verify.sql'
    if (-not (Test-Path $verifyFile)) { throw "Khong tim thay $verifyFile" }

    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t6_verify.sql"
    ...
    foreach ($line in $output) { Write-Host $line }
    $text = $output -join [Environment]::NewLine
    if ($exitCode -ne 0) { throw "T6 SQL verify fail, psql exit=$exitCode`n$text" }
    if ($text -notmatch 'T6 FINAL CORE CHECKS: PASS') { throw 'Khong tim thay marker T6 PASS.' }
    return $text
}

try {
    Write-Host '=== HomeTechVN T6 v1.0 - Checklist Engine Verification ==='
    ...
    $sqlOutput = Invoke-T6SqlVerify -Container $container
    ...
    $snapshotPath = Join-Path $snapshotDir ('T6_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')
    $snapshot = @(
        'HomeTechVN T6 local verification',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T6 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T6 FINAL CORE CHECKS: PASS'
    Write-Host 'T6 APP BUILD: PASS'
    ...
    Write-Host ''
    Write-Host '[T6 FAIL]' -ForegroundColor Red
