...
function Assert-Migrations {
    ...
    if ($unexpected.Count -gt 0) {
        throw ("Co migration ngoai checkpoint T13:`n- " + ($unexpected -join "`n- "))
    }
    Write-Host '[PASS] Migration T1+T2+T3+T4+T5+T6+T7+T8+T9+T10+T11+T12+T13: 33/33'
    return $actual
}
...
function Invoke-T13SqlVerify {
    ...
    Write-Host ''
    Write-Host "> docker.exe exec -i $Container psql ... < t13_verify.sql"
    $previous = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $output = @(
    ...
    if ($exitCode -ne 0) {
        throw "T13 SQL verify fail, psql exit=$exitCode`n$text"
    }
...
try {
    Write-Host '=== HomeTechVN T13 v1.0 - Reports Verification ==='
    ...
    $sqlOutput = Invoke-T13SqlVerify -Container $container
    ...
    Write-Host '[PASS] T12 public QR warranty regression checks'

    Invoke-CheckedCommand -FilePath $node -Arguments @((Join-Path $ProjectRoot 'scripts\t13-ui-check.mjs')) | Out-Null
    Write-Host '[PASS] T13 Reports responsive/privacy checks'
    ...
    $snapshotPath = Join-Path $snapshotDir ('T13_LOCAL_VERIFY_' + (Get-Stamp) + '.txt')

    $snapshot = @(
        'HomeTechVN T13 local verification',
        ...
        $sqlOutput,
        '',
        'T11 GLOBAL RESPONSIVE UI REGRESSION: PASS',
        'T12 PUBLIC WARRANTY REGRESSION: PASS',
        'T13 REPORT RESPONSIVE UI: PASS',
        'APP BUILD: PASS',
    ...
    Write-Host ''
    Write-Host '=========================================='
    Write-Host 'T13 LOCAL REPRODUCIBILITY: PASS'
    Write-Host 'T13 FINAL CORE CHECKS: PASS'
    Write-Host 'T13 RESPONSIVE UI CHECK: PASS'
    Write-Host 'T13 APP BUILD: PASS'
    Write-Host "Snapshot: $snapshotPath"
    Write-Host '=========================================='
    exit 0
} catch {
    Write-Host ''
    Write-Host '[T13 FAIL]' -ForegroundColor Red
